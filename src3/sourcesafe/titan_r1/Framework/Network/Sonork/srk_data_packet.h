...
, SONORK_SERVER_CLIENT_EVENT_NETWORK_CONFIG
};
...
public:
	...
	TSonorkTaskId&
		ReqTaskId()
		{ return header.packet_info.req.taskId;}
	...
	TSonorkTaskId&
		AknTaskId()
		{ return header.packet_info.akn.taskId;}

	SONORK_RESULT
		AknResult();

	BOOL	IsAknEnd() const
		{ return header.packet_info.req.taskId.Server()==SONORK_INVALID_TASK_ID;}
...
// User Data & Search
	...
	UINT
		E_CreateUser1_R(UINT sz
			,DWORD referrer_id
			,const TSonorkUserDataNotes&
			,const TSonorkDynData&);
...
// Depreciated
//	UINT	E_AddService_R(UINT sz,const TSonorkServiceData&);

//	bool	D_AddService_A(UINT sz,TSonorkServiceLocus1& service_locus);

//	UINT	E_NewListServices_R(UINT sz
...
//	bool	D_ListServices_A(UINT sz,TSonorkServiceDataQueue&);
...
// Messages
	...
	UINT
		E_PutMsg_R(UINT sz,const TSonorkLocus1*locus,const TSonorkMsg*msg);
...
// Control Messages
	...
	bool
		D_CtrlMsg_E(UINT P_size, TSonorkLocus1*, TSonorkCtrlMsg*,TSonorkDynData*);

//	bool	D_OldSid_E(UINT size,TSonorkOldSidNotificationList*);
	...
	UINT
		E_SetSid_R(UINT sz, const TSonorkLocusAtom& , const TSonorkDynString& msg);

	bool
		D_SetSid_A(UINT sz , TSonorkLocusAtom& );
...
// User Data & Search
	...
	bool
		D_SetUserData_R(UINT sz
			, DWORD sidVersion
			, TSonorkUserDataNotes&);

	UINT
		E_SetUserData_A(UINT sz
			, const TSonorkSerial&
			);
	...
	bool
		D_CreateUser0_R(UINT sz
			, DWORD sidVersion
			, TSonorkUserDataNotes&);

	bool
		D_CreateUser1_R(UINT sz
			, DWORD sidVersion
			, DWORD&referrer_id
			, TSonorkUserDataNotes&
			, TSonorkDynData&);
...
// Groups
	...
	bool
		D_SetGroup_R2(UINT sz
			, DWORD sidVersion
			, TSonorkGroup&UG);

	UINT
		E_SetGroup_A2(UINT sz
			, DWORD sidVersion
			, DWORD subfunc
...
// Services
	...
	bool
		D_AddService_R(UINT sz
			, DWORD sidVersion
			, TSonorkServiceData&);

	UINT
		E_AddService_A(UINT sz
			, DWORD sidVersion
			, const TSonorkServiceLocus1& svc_locus);
	...
	UINT
		E_ListServices_A(UINT sz
			, DWORD sidVersion
			,TSonorkServiceDataQueue&);

// ----------------------------------------------------------------------------
// User servers

	bool
		D_AddUserServer_R(UINT sz
			, DWORD sidVersion
			, TSonorkUserServer&);

	bool
		D_SetUserServer_R(UINT sz
			, DWORD sidVersion
			, TSonorkUserServer&);

	bool
		D_ListUserServers_R(UINT sz
			, DWORD sidVersion
			, TSonorkUserServer& data, DWORD& flags, DWORD& max_count);

	UINT
		E_ListUserServers_A(UINT sz
			, DWORD sidVersion
			, TSonorkUserServerQueue&);

	bool
		D_DelUserServer_R(UINT sz
			, TSonorkSid&,DWORD&);
...
// Messages
	...
	// Ctrl msg event
	UINT
		E_CtrlMsg_E(UINT A_size
			, DWORD sidVersion
			, const TSonorkLocus1*, const void*data, UINT data_size);

	// Old Sid event
	UINT
		E_OldSid_E(UINT size
			, UINT  items
	...
	UINT
		E_Sid_E(UINT size
			, DWORD sidVersion
			, const TSonorkSidNotification&);

	UINT
		E_Sid_E(UINT size
			, DWORD sidVersion
			,TSonorkSidNotificationQueue&);

	// Services Event
	UINT
		E_NetworkConfig_E(UINT size
			, DWORD sidVersion
			, DWORD serviceCount
			, const TSonorkServiceLocus*	serviceList
			, DWORD sourceNetworkId
			, const TSonorkQueue*		networkList
			, DWORD maxNetworks
			);
	...
	bool
		D_GetMsg_R(UINT size
			, TSonorkObjId *msg_id,DWORD*ctrl_flags);

	UINT
		E_GetMsg_A(UINT size
			, DWORD sidVersion
			, const TSonorkMsg*);

	bool
		D_DelMsg_R(UINT size
			, DWORD& count
			, TSonorkObjId *msg_id);

	bool
		D_WappUrl_R(UINT sz
			, DWORD*app_no,DWORD*app_id);
...
// Trackers
	...
	UINT
		E_Data_E(UINT A_size
			, DWORD sidVersion
			, TSonorkDynData&);

	bool
		D_Data_E(UINT P_size
			, DWORD sidVersion
			,TSonorkDynData&);

	UINT
		E_Error_A(UINT A_size
			, SONORK_FUNCTION, const TSonorkError&E);

	bool
		D_Error_A(UINT P_size
			, TSonorkError&E);
...
// does extra maintenance work on them.
	...
	static TSonorkDataPacket *
		Alloc(DWORD dataSize);

	void   	Free(){SONORK_MEM_FREE(this);}

}__SONORK_PACKED;

#if defined(SONORK_CLIENT_BUILD)
extern DWORD sonorkServerVersion;
#endif
