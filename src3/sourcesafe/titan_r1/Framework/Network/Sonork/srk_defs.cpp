 ...
 TSonorkSidFlags::xCopyUserControlled(TSonorkSidFlags& O)
{
	v[0].v[0]=O.v[0].v[0];
	v[1].v[0]=O.v[1].v[0];
	v[0].v[1]=(v[0].v[1]&~SONORK_SID_FM1_USER_CONTROLLED)|(O.v[0].v[1]&SONORK_SID_FM1_USER_CONTROLLED);
}
void
 TSonorkSidFlags::xClearServerControlled()
{
	v[0].v[1]&=SONORK_SID_FM1_USER_CONTROLLED;
	v[1].v[1]=0;
}
void
...
SONORK_C_CSTR
 TSonorkShortString::SetMax(SONORK_C_CSTR newStr,UINT maxLen)
{
	UINT strLen;
	if(str)Clear();
	if(newStr)
	{
		...
		if(strLen>maxLen)
			strLen=maxLen;
		str=SONORK_MEM_ALLOC(char,strLen<8?8:strLen+1);
		memcpy(str,newStr,strLen);
		*(str+strLen)=0;
	}
	return str;
}
...
SONORK_C_CSTR
 TSonorkShortString::Set(SONORK_C_CSTR newStr)
{
	UINT sz;
	if(str)Clear();
	if(newStr)
...
// ----------------------------------------------------------------------------
void
 TSonorkCtrlMsg::LoadCmd(
			  SONORK_CTRLMSG_CMD	pCmd
			, DWORD 		userParam1
			, DWORD 		userParam2
			, DWORD			pSysFlags)
{
	cmdFlags		= pCmd;
	sysFlags		= pSysFlags|SONORK_CTRLMSG_SF_NEW;
	body.n.usrParam.Set(userParam1,userParam2);
...
UINT
 SONORK_StrCopy(SONORK_W_STR target,UINT target_length,SONORK_C_CSTR source,UINT source_length)
{
