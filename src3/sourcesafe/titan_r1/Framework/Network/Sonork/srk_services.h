...
// P_size is the size of the while packet, as received by the tcp engine
	DWORD	E_Req(DWORD	D_size, UINT function, const TSonorkCodecAtom*,DWORD codecVersion);
	bool	D_Req(DWORD	P_size, TSonorkCodecAtom*,DWORD codecVersion);
	DWORD	E_Akn(DWORD	D_size, UINT function, const TSonorkCodecAtom*,DWORD codecVersion);
	bool    D_Akn(DWORD	P_size, TSonorkCodecAtom*,DWORD codecVersion);	// should NOT call if IsError() is set
...
public:
	...
	DWORD	CODEC_DataSize(DWORD codecVersion=0)	const
...
enum SONORK_DATA_SERVER_LOGIN_FLAGS
{
  SONORK_DATA_SERVER_LOGIN_F_CORRECTED_HEADER	= 0x00010000
};
...
public:
	DWORD
		ServerNo() const
		{ return header.serverNo; }

	SONORK_SERVICE_TYPE
		ServiceType() const
		{ return (SONORK_SERVICE_TYPE)header.serviceType;}
...
const	SONORK_DWORD4&
	...
	SONORK_PIN_TYPE
		PinType() const
		{ return (SONORK_PIN_TYPE)header.pinType; }
