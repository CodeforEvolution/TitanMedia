...
public:
	...
	SONORK_PAIR_VALUE_TYPE
		Type() const
		{ return type;}

	void
		SetType(SONORK_PAIR_VALUE_TYPE);
	...
	bool
		GetDW2(SONORK_DWORD2&) const;

	SONORK_DWORD2&
		SetDW2(const SONORK_DWORD2&);

	TSonorkShortString*
		SetStr(SONORK_C_CSTR str);
...
const	TSonorkShortString*
	...
	TSonorkDynData*
		SetDynData(const void*ptr=NULL,UINT sz=0,SONORK_ATOM_TYPE tp=SONORK_ATOM_NONE);

	TSonorkDynData*
		SetDynDataAtom(TSonorkCodecAtom*A, DWORD codecVersion=SONORK_CURRENT_CODEC_VERSION);

// --------------
// PSEUDO CODEC (TSonorkShortString is not a TSonorkCodecAtom)

	UINT
		CODEC_Size(DWORD codecVersion) const;

	void
		CODEC_WriteDataMem(TSonorkCodecWriter& CODEC) const;

	void
		CODEC_ReadDataMem(TSonorkCodecReader& CODEC);
...
public:
	...
	TSonorkPairValue *
		Add(TSonorkPairValue *PV)
		{ return (TSonorkPairValue*)w_Add(PV); }

	bool
		Remove(SONORK_C_CSTR name);

	bool
		Remove(TSonorkPairValue *PV)
		{ return w_Remove(PV); }


	TSonorkPairValue *
		Get(SONORK_C_CSTR name);

	TSonorkPairValue *
		GetIndexed(SONORK_C_CSTR name,UINT idx,SONORK_PAIR_VALUE_TYPE = SONORK_PAIR_VALUE_ANY);


	TSonorkPairValue *
		GetType(SONORK_C_CSTR name, SONORK_PAIR_VALUE_TYPE);
...
private:
	...
	DWORD	CODEC_DataSize(DWORD codecVersion) const;
