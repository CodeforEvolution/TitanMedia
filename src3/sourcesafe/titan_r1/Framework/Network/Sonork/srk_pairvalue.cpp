...
DWORD
 TSonorkPairValueQueue::CODEC_DataSize(DWORD codecVersion) const
{
	TSonorkListIterator I;
	TSonorkPairValue *	PV;
	DWORD			size;
	...
	if( Items() )
	{
		...
		while( (PV=EnumNext(I)) != NULL )
			size+=PV->CODEC_Size(codecVersion);
		EndEnum(I);
	}
	return size;
}
...
TSonorkDynData*
 TSonorkPairValue::SetDynDataAtom(TSonorkCodecAtom*A, DWORD codecVersion)
{
	SetType(SONORK_PAIR_VALUE_DYN_DATA);
	if( A )
	{
		A->CODEC_Write( value.d_data , codecVersion);
	}
	else
	{
		value.d_data->Clear();
...
UINT
 TSonorkPairValue::CODEC_Size(DWORD ) const
{
	UINT size;
	...
	switch( type )
	{
		...
		case SONORK_PAIR_VALUE_DYN_DATA:
			size+=value.d_data->CODEC_Size(SONORK_NULL_CODEC_VERSION);
			break;
	}
	return size;
}
void
 TSonorkPairValue::CODEC_WriteDataMem(TSonorkCodecWriter& CODEC) const
{
