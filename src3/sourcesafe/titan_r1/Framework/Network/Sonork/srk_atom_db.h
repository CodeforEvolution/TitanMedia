...
public:
	...
	DWORD
		Items()		const
		{ return db.header.items;}

	SONORK_RESULT
		Open(const TSonorkId&, const char *name, bool& reset, bool deleteOnClose);
	...
	DWORD
		GetValue(SONORK_ATOM_DB_VALUE idx) const
		{ return db.header.value[idx]; }

	SONORK_RESULT
		SetValue(SONORK_ATOM_DB_VALUE idx, DWORD value);


	SONORK_RESULT
		Add(const TSonorkCodecAtom*,DWORD*new_index=NULL,TSonorkTag*tag=NULL);

	SONORK_RESULT
		SetTag(DWORD index, TSonorkTag&tag);

	SONORK_RESULT
		Get(DWORD index, TSonorkCodecAtom*A=NULL,TSonorkTag*tag=NULL);

	SONORK_RESULT
		Del(DWORD index);

	SONORK_RESULT
		GetEntryStatus(DWORD index, DWORD& size , BOOL& deleted );

	SONORK_RESULT
		AddRaw(TSonorkDynData& DD,TSonorkTag& tag,DWORD& new_index);

	SONORK_RESULT
		GetRaw(DWORD index, TSonorkDynData& DD,TSonorkTag& tag);
