...
const	TSonorkP2Mapping*
		GetNetworkMapping(DWORD targetId);
...
const	TSonorkHostInfo&
	...
	void
		...
		{ control.socks_info.SetV4(physAddr); }
	bool
		IsUsingSocks()	const;
	...
	const TSonorkPhysAddr&
		HostAddr() const
		{ return control.hostAddr; }

	int
		GetLocalAddr(TSonorkPhysAddr& physAddr);
