...
EMVideoFileInputDescriptor::EMVideoFileInputDescriptor(string p_oFileName) 
	:	EMFileInputDescriptor(p_oFileName, EM_TYPE_ANY_VIDEO),
		m_opVideoFile(NULL)
{
//	EMMediaUtility::push(this, "EMVideoFileInputDescriptor");
}

EMVideoFileInputDescriptor::~EMVideoFileInputDescriptor()
{
	delete m_opVideoFile;
//	EMMediaUtility::pop("EMVideoFileInputDescriptor");
}

EMMediaFormat* EMVideoFileInputDescriptor::GetFormat()
{
	return m_opVideoFile -> GetFormat();
}

bool EMVideoFileInputDescriptor::InitCheckE()
{
...
//	int64 vVideoTime = EMBeMediaUtility::FramesToTime(vNumVideoFrames, opFormat);
	...
	m_vNumFrames = EMMediaUtility::Instance() -> TimeToFrames(m_opVideoFile -> GetDuration(), EMMediaUtility::Instance() -> GetSystemAudioFormat());
	return true;
}

bool EMVideoFileInputDescriptor::Open()
{
	if(GetType() == EM_TYPE_RAW_VIDEO || GetType() == EM_TYPE_ANY_VIDEO || GetType() == EM_TYPE_ENCODED_VIDEO)
	{
		if(m_opVideoFile == NULL)
...
bool EMVideoFileInputDescriptor::ReadFrames(EMMediaDataBuffer** p_opPrimaryBuffer, EMMediaDataBuffer** p_opSecondaryBuffer, int64 p_vMediaFramePosition, int64 p_vOffsetFrames, int64 & p_vOutNumRead, bool p_vSeeking)
{
	//Convert the offset value from "frames" into "bytes"
	int64 vOffset = p_vOffsetFrames;// EMBeMediaUtility::FramesToBytes(p_vOffsetFrames, m_opVideoFile -> GetFormat());
	int64 vMediaTimePosition = EMMediaUtility::Instance() -> FramesToTime(p_vMediaFramePosition, EMMediaUtility::Instance() -> GetSystemAudioFormat());// m_opVideoFile -> GetFormat());
...
bool EMVideoFileInputDescriptor::SeekTo(int64 p_vNewMediaFrame)
{
	m_opVideoFile -> SeekToFrame(p_vNewMediaFrame);
	return true;
}

bool EMVideoFileInputDescriptor::SeekAndDisplay(int64 p_vNewMediaFrame)
{
	//m_opVideoFile -> SeekToTime(p_vNewMediaFrame);
	EMDebugger("EMVideoFileInputDescriptor::SeekAndDisplay(int64 p_vNewMediaFrame)");
	return true;
}
