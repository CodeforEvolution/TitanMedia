...
#ifndef __EM_WIN_AUDIO_CLIP_REPOSITORY
#define __EM_WIN_AUDIO_CLIP_REPOSITORY
...
class EMWinAudioClipRepository : public EMMediaClipRepository
{
public:
	EMWinAudioClipRepository();
	~EMWinAudioClipRepository();
	int32 GetID() const;
	void GetNextBuffers(list<EMMediaDataBuffer*>* p_opList, EMMediaType p_eType, int64 p_vTimeNow, bool p_vSeeking = false);
	uint32 GetNextBuffers(EMMediaDataBuffer** p_opArray, EMMediaType p_eType, int64 p_vTimeNow, bool p_vSeeking = false);
	bool InitCheckE();
	void AdjustToBPM(uint32 p_vBPM);
