...
// TSonorkNetIO
// ----------------------------------------------------------------------------
void	TSonorkNetIO::SetBufferSize(UINT sz)
{
	if( sz != buffer.size )
	{
...
int TSonorkNetIO::_Accept(SOCKET *new_sk,sockaddr_in*sender_address, bool blocking)
{
	...
	*new_sk=accept(Socket(),(sockaddr*)sender_address,&addr_len);
	...
	err_code=SetSocketBlocking(*new_sk,blocking);
	if(err_code)
	{
		TSonorkNetIO::CloseSocket(*new_sk);
		return err_code;
	}
	return 0;
}
 ...
 TSonorkNetIO::CreateSocket(bool blocking, long send_size,long recv_size )
{
	...
	{
		int rv;
		rv=SetSocketBlocking(sk,blocking);
		if(rv)
		{
			//Migs
			TSonorkNetIO::CloseSocket(sk);
			sk=INVALID_SOCKET;
			return rv;
		}
	}
...
{
	return SetSocketBufferSize(Socket(),send_size,recv_size );
}
...
int
 TSonorkNetIO::SetSocketBufferSize(SOCKET sk, long send_size,long recv_size )
{
	long  result_val;
	SONORK_SOCK_ADDR_LEN	valueSize;
//	sonork_printf("SetBufferSize(%x,%d,%d)",sk,send_size,recv_size);


	if( send_size >= 128)
	{
		valueSize=sizeof(result_val);
		if(getsockopt(sk,SOL_SOCKET,SO_SNDBUF,(char*)&result_val,&valueSize))
			return GetLastError();
//		sonork_printf(" CurrentSend=%d",result_val);

		valueSize=sizeof(valueSize);
		setsockopt(sk,SOL_SOCKET,SO_SNDBUF,(char*)&send_size,valueSize);
	}

	if( recv_size >= 128 )
	{
		valueSize=sizeof(result_val);
		if(getsockopt(sk,SOL_SOCKET,SO_RCVBUF,(char*)&result_val,&valueSize))
			return GetLastError();
//		sonork_printf(" CurrentRecv=%d",result_val);

		valueSize=sizeof(recv_size);
		setsockopt (sk,SOL_SOCKET,SO_RCVBUF,(char*)&recv_size,valueSize);
	}
	return 0;
}

int
 TSonorkNetIO::SetSocketBlocking(SOCKET sk, bool blocking)
{
//Oliver,Migs
#if defined(SONORK_WIN32_BUILD)
...
int
 TSonorkNetIO::GetSocketAddr(SOCKET sk,TSonorkPhysAddr& pA)
{
	SONORK_SOCK_ADDR_LEN	valueSize;
	int errCode;
	valueSize=sizeof(sockaddr);
	if((errCode = getsockname(sk,(sockaddr*)&pA.data.inet1.addr,&valueSize))==0)
	{
		pA.SetType(SONORK_PHYS_ADDR_TCP_1);
	}
	else
	{
		pA.Clear();
...
BOOL
 TSonorkNetIO::Start(BYTE hv, BYTE lv)
{
#if defined(SONORK_WIN32_BUILD)
	WSADATA wsadata;
	WORD version;
...
bool
  TSonorkNetIO::IsPrivateInetAddr(DWORD addr)
{
...
SONORK_C_STR
  TSonorkNetIO::GetStatusName(SONORK_NETIO_STATUS s)
{
static SONORK_C_CSTR
	sonork_netio_status_name[SONORK_NETIO_STATUS_ITEMS]=
	{
	 "Disconnected"
	,"Disconnecting"
	,"Req-Connect"
    ...
    if(s>=SONORK_NETIO_STATUS_DISCONNECTED&&s<SONORK_NETIO_STATUS_ITEMS)
		return (SONORK_C_STR)sonork_netio_status_name[s];
	else
		return "??";
}
...
bool TSonorkPhysAddr::IsEqual(sockaddr_in*src) const
{
	if(Type()!=SONORK_PHYS_ADDR_NONE)
	{
		...
		return	data.inet1.addr.sin_addr.s_addr==src->sin_addr.s_addr
			&&
...
int
 TSonorkPhysAddr::SetInet1(SONORK_PHYS_ADDR_TYPE t,SONORK_C_CSTR p_host_name,WORD port)
{
	int err;
...
int
 TSonorkPhysAddr::SetInet1(SONORK_PHYS_ADDR_TYPE t, const sockaddr_in*src)
{
	if(t==SONORK_PHYS_ADDR_TCP_1||t==SONORK_PHYS_ADDR_UDP_1)
	{
		header.type=(BYTE)t;
...
SONORK_PHYS_ADDR_TYPE
 TSonorkPhysAddr::GetInet1(sockaddr_in*src)
{
	if(Type()==SONORK_PHYS_ADDR_TCP_1||Type()==SONORK_PHYS_ADDR_UDP_1)
	{
...
void
 TSonorkPhysAddr::SetType(SONORK_PHYS_ADDR_TYPE t, UINT family)
{
	header.type=(BYTE)t;
...
static const char *sonorkPhysAddrPrefix[SONORK_PHYS_ADDR_TYPES]={"","tcp:","udp:"};
...
{
	if(*((DWORD*)S)==*((DWORD*)sonorkPhysAddrPrefix[SONORK_PHYS_ADDR_TCP_1]))
	header.type=SONORK_PHYS_ADDR_TCP_1;
    else
	if(*((DWORD*)S)==*((DWORD*)sonorkPhysAddrPrefix[SONORK_PHYS_ADDR_UDP_1]))
...
void
 TSonorkHostInfo::SetV4(const TSonorkPhysAddr* pA)
{
	if( pA )
	if( pA->IsValid(SONORK_PHYS_ADDR_TCP_1,false) )
	{
		version  = 4;
		physAddr.Set( *pA );
		return;
