	...
	, const TSonorkError*	pERR)
{
	...
	else
	{
		switch( function )
		{
			...
			{
				profile_user.addr.header.sidFlags.Set( sonork.SidFlags() );
				profile.ctrl_data.header.sidFlags.Set( sonork.SidFlags() );
	...
	,DWORD flags)
{
	...
	{
		profile_user.addr.header.sid.Set( sonork.Sid() );
		profile_user.addr.header.sidFlags.Set( sonork.SidFlags() );
		profile_user.addr.hostAddr.Set( sonork.HostAddr() );
		profile_user.addr.utsAddr.Clear();
		sonork_printf("* ** SONORK_SID IS %x.%x",profile_user.addr.header.sid.v[0],profile_user.addr.header.sid.v[1]);
	}
	profile_user.addr.header.version.Set( app.version );
...
void	TSonorkAppBase::PSE_UserSid(
		const TSonorkLocusAtom*		nLocus
	, 	const TSonorkSerial*		uSerial
	,	      TSonorkDynString*		sText)
{
	TSonorkExtUserData *extUD;
	extUD=UserList().Get(nLocus->userLocusEx.userId);
	if(extUD!=NULL)
	{
		TSonorkLocusAtom 	oLocus;

		extUD->GetLocus(&oLocus);
		if(nLocus->SidMode() != SONORK_SID_MODE_DISCONNECTED)
		{
			extUD->RunValue(SONORK_URV_CURRENT_SID_SEQ_NO)+=1;
			extUD->addr.header.sid.Set(nLocus->userLocusEx.sid);
			extUD->addr.header.sidFlags.Set(nLocus->userLocusEx.sidFlags);
			extUD->addr.header.version.Set(nLocus->userLocusEx.version);
			extUD->addr.utsAddr.Set(nLocus->userLocusEx.utsAddr);
			extUD->addr.hostAddr.Set(nLocus->userLocusEx.hostAddr);
			extUD->wRegion().Set(nLocus->userLocusEx.region);
			extUD->sid_text.Set( sText->ToCStr() );
			extUD->ctrlData.lastOnlineTime.SetTime_Local();
			if( !uSerial->IsZero() )
		...
		else
		{
			...
			if( extUD->addr.SidMode() == SONORK_SID_MODE_DISCONNECTED)
				return;
			extUD->addr.Clear();
		}
			...
			, SONORK_C_CSTR password)
{
	...
	SONORK_RESULT 		result;
	TSonorkShortString	savedServerProfileName;

	// OpenProfile will overwrite the current server profile name
	// we save it here.

	savedServerProfileName.Set(  profile.ctrl_data.serverProfile );
	...
	if(result==SONORK_RESULT_OK)
	{
		...
		profile.ctrl_data.serverProfile.Set( savedServerProfileName );
	...
	, SONORK_C_CSTR open_password)
{
	...
	if( result == SONORK_RESULT_OK )
	{
		...
		else
		{
			...
			{
				if( strcmp(open_password?open_password:""
					,profile_password.CStr()) )
				{
...
SONORK_RESULT
 TSonorkAppBase::OpenProfile(TSonorkId& userId,SONORK_C_CSTR password)
{
	SONORK_RESULT	result;
