...
TSonorkRawTcpEngine::TSonorkRawTcpEngine(DWORD buffer_size)
...
DWORD
 TSonorkRawTcpEngine::Send(const void*data,DWORD data_size)
{
	TSonorkRawTcpPacket *P;

	P=TSonorkRawTcpPacket::Alloc(data_size);
...
TSonorkPacketTcpEngine::TSonorkPacketTcpEngine(DWORD buffer_size)
:TSonorkTcpEngine(buffer_size)
{
	o_packet=i_packet=NULL;
	o_offset=i_offset=0;
}
TSonorkPacketTcpEngine::~TSonorkPacketTcpEngine()
{
	if(i_packet)i_packet->Free();
	if(o_packet)o_packet->Free();
}
		...
		do
		{
			if(i_offset<SONORK_TCP_HEADER_SIZE)
			{
				...
				if(i_offset>=SONORK_TCP_HEADER_SIZE)
				{
					...
					i_packet=TSonorkTcpPacket::Alloc( i_header.size );
...
{
	TSonorkTcpPacket*P;
	P=TSonorkTcpPacket::Alloc(data_size);
...
void  TSonorkTcpEngine::SendSocksConnectRequest()
{
	...
	else
	{
		...
		if(socks_info.physAddr.Inet1()->sin_addr.s_addr == 0)
			error = 30;
		else
...
TSonorkTcpPacket *
 TSonorkTcpPacket::Alloc(DWORD data_size)
{
...
TSonorkRawTcpPacket*
 TSonorkRawTcpPacket::Alloc(DWORD data_size)
{
