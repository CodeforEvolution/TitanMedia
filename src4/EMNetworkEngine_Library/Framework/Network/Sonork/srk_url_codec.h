...
inline UINT
	SONORK_UrlMaxEncodeSize(UINT sz){ return (sz*3)+1;}

inline UINT
	SONORK_UrlMaxDecodeSize(UINT sz){ return sz+1;}

SONORK_C_CSTR
	SONORK_UrlEncode(SONORK_C_STR pTgt, UINT pTgtSize, SONORK_C_CSTR str);

SONORK_C_CSTR
	SONORK_UrlDecode(SONORK_C_STR pTgt, UINT pTgtSize, SONORK_C_CSTR pSrc, UINT pSrcLen=(UINT)-1);

SONORK_C_CSTR
	SONORK_UrlEncode(TSonorkShortString* pTgt, SONORK_C_CSTR str);

SONORK_C_CSTR
	SONORK_UrlDecode(TSonorkShortString* pTgt, SONORK_C_CSTR pSrc, UINT pSrcLen=(UINT)-1);

SONORK_C_CSTR
	SONORK_UrlEncode(TSonorkTempBuffer*pTgt, SONORK_C_CSTR str);

SONORK_C_CSTR
	SONORK_UrlDecode(TSonorkTempBuffer*pTgt, SONORK_C_CSTR pSrc, UINT pSrcLen=((UINT)-1));

SONORK_C_CSTR
	SONORK_UrlParamEncode(TSonorkTempBuffer*pTgt, SONORK_C_CSTR name,const SONORK_DWORD2&value);

SONORK_C_CSTR
	SONORK_UrlParamEncode(TSonorkTempBuffer*pTgt, SONORK_C_CSTR name,SONORK_C_CSTR value);

SONORK_C_CSTR
	SONORK_UrlEncode(SONORK_C_STR tgt, UINT tgt_size, SONORK_C_CSTR str);


void
	SONORK_HtmlPuts(FILE*,SONORK_C_CSTR);
