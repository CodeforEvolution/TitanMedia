...
SONORK_RESULT
 CODEC_WriteFileStreamDW(SONORK_FILE_HANDLE handle, const TSonorkCodecAtom*A,DWORD& codec_size,int *p_err_code,DWORD crypt_code)
{
	int 		err_code;
	SONORK_RESULT	result;
	BYTE*		buffer;
	codec_size = A->CODEC_Size(SONORK_CURRENT_CODEC_VERSION);
	if( (err_code = SONORK_IO_WriteFile(handle,&codec_size,sizeof(DWORD))) == 0 )
	{
		buffer 	= SONORK_MEM_ALLOC(BYTE,codec_size);
		result	= A->CODEC_WriteMem(buffer,codec_size,SONORK_CURRENT_CODEC_VERSION);
...
SONORK_RESULT
  CODEC_ReadFileStreamDW(SONORK_FILE_HANDLE handle,TSonorkCodecAtom*A , DWORD& codec_size , int *p_err_code , DWORD crypt_code)
{
	SONORK_RESULT 	result;
	BYTE  		*buffer;
	int 		err_code;
	...
	if( (err_code = SONORK_IO_ReadFile(handle,&codec_size,sizeof(DWORD))) == 0 )
	{
		...
		else
		{
			...
			if( (err_code=SONORK_IO_ReadFile(handle,buffer,codec_size)) == 0 )
			{
				...
				result=A->CODEC_ReadMem(buffer,codec_size,SONORK_CURRENT_CODEC_VERSION);
			}
			else
			{
				result=SONORK_RESULT_STORAGE_ERROR;
