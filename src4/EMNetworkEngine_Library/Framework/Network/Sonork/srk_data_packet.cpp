...
#define DECODE_AKN_SECTION_STARTS(psize) \
			TSonorkPacketReader CODEC(DataPtr(),(psize)-sizeof(header), sonorkServerVersion);\
...
#define DECODE_REQ_SECTION_STARTS(sz)	TSonorkPacketReader CODEC(DataPtr(),(sz)-sizeof(header),sidVersion)

#define DECODE_NV_REQ_SECTION_STARTS(sz)	TSonorkPacketReader CODEC(DataPtr(),(sz)-sizeof(header),SONORK_V15_COMPATIBLE_CODEC_VERSION)
...
#define ENCODE_REQ_SECTION_STARTS(func,psize,subfunc_ver)\
			...
			TSonorkPacketWriter CODEC(DataPtr(),psize,sonorkServerVersion);\
			{

#define ENCODE_EVT_SECTION_STARTS(evt,psize,subfunc_ver)\
			InitHeader(evt,subfunc_ver);\
			TSonorkPacketWriter  CODEC(DataPtr(),psize,sidVersion);\
			{\

#define ENCODE_NV_EVT_SECTION_STARTS(evt,psize,subfunc_ver)\
			InitHeader(evt,subfunc_ver);\
			TSonorkPacketWriter  CODEC(DataPtr(),psize,SONORK_V15_COMPATIBLE_CODEC_VERSION);\
			{\

#define ENCODE_NV_AKN_SECTION_STARTS(func,psize,subfunc_ver)\
...
DWORD sonorkServerVersion;
#endif

inline void
 TSonorkDataPacket::InitHeader(SONORK_FUNCTION pFunction,DWORD subfunc_ver_type)
{
...
inline void
 TSonorkDataPacket::InitHeader(SONORK_SERVER_CLIENT_EVENT pEvent,DWORD subfunc_ver)
{
...
bool TSonorkDataPacket::D_GetSysMsgs_R(UINT sz
	, DWORD sidVersion
	, TSonorkTime& time
...
#if defined(SONORK_CLIENT_BUILD)
UINT TSonorkDataPacket::E_GetSysMsgs_R(UINT sz
	, const TSonorkTime& time
...
bool TSonorkDataPacket::D_GetSysMsgs_A(UINT sz
...
bool TSonorkDataPacket::D_WappUrl_R(UINT sz
	, DWORD*app_no
	, DWORD* app_id)
{
	DECODE_NV_REQ_SECTION_STARTS(sz);
		CODEC.ReadDW(app_no);
		CODEC.ReadDW(app_id);
	DECODE_REQ_SECTION_ENDS;
}
UINT TSonorkDataPacket::E_WappUrl_A(UINT sz
	...
	,DWORD app_pin)
{
	ENCODE_NV_AKN_SECTION_STARTS(SONORK_FUNCTION_WAPP_URL,sz,GUDPCV_WAPP_URL);
		CODEC.Write(&url);
...
#if defined(SONORK_SERVER_BUILD)
bool TSonorkDataPacket::D_CreateUser0_R(UINT sz
	, DWORD sidVersion
...
{
	ENCODE_NV_AKN_SECTION_STARTS(SONORK_FUNCTION_CREATE_USER,sz,0);
...
bool
 TSonorkDataPacket::D_AddUserServer_R(UINT sz
	,DWORD sidVersion
...
bool
 TSonorkDataPacket::D_SetUserServer_R(UINT sz
	,DWORD sidVersion
 	,TSonorkUserServer& data)
{
	return D_AddUserServer_R(sz,sidVersion,data);
}
#endif
...
#if defined(SONORK_SERVER_BUILD)
bool
 TSonorkDataPacket::D_ListUserServers_R(UINT sz
	, DWORD sidVersion
	, TSonorkUserServer& data
	, DWORD& flags
	, DWORD& max_count)
...
UINT
 TSonorkDataPacket::E_ListUserServers_A(UINT sz
	, DWORD sidVersion
...
bool
 TSonorkDataPacket::D_AddService_R(UINT sz
	, DWORD sidVersion
...
UINT TSonorkDataPacket::E_ListServices_A(UINT sz
	, DWORD sidVersion
	, TSonorkServiceDataQueue& list)
{
...
// DEPRECIATED
bool
 TSonorkDataPacket::D_ListServices_A(UINT sz, TSonorkServiceDataQueue& queue)
{
	DWORD 		items;
	TSonorkServiceData*data;
	DECODE_AKN_SECTION_STARTS(sz);
...
bool
 TSonorkDataPacket::D_SearchUser_R_NEW(UINT sz
	, DWORD sidVersion
	, TSonorkUserDataNotes&	UDN
	, DWORD& search_flags
	, DWORD& max_count)
...
bool
 TSonorkDataPacket::D_SearchUser_R_OLD(UINT sz
	, DWORD sidVersion
	, TSonorkId& user_id
...
bool
 TSonorkDataPacket::D_SetSid_R(UINT size
	, DWORD 		sidVersion
	, TSonorkLocusAtom& 	locusAtom
	, TSonorkDynString& 	sidText)
{
	DECODE_REQ_SECTION_STARTS(size);
		if( sidVersion >= SONORK_EXTENDED_CODEC_VERSION_1 )
		{
			CODEC.Read(&locusAtom);
			CODEC.Read(&sidText);
		}
		else
		{
			CODEC.ReadDW4(&locusAtom.userLocusEx.sidFlags);
			CODEC.ReadDW2(&locusAtom.userLocusEx.region);
			CODEC.ReadPA1(&locusAtom.userLocusEx.utsAddr);
...
UINT TSonorkDataPacket::E_SetSid_A(UINT sz
	, DWORD 			sidVersion
	, const TSonorkLocusAtom&	locusAtom)
{
	ENCODE_AKN_SECTION_STARTS(SONORK_FUNCTION_SET_SID,sz,GUDPCV_SET_SID);
		if( sidVersion >= SONORK_EXTENDED_CODEC_VERSION_1 )
		{
			CODEC.Write(&locusAtom);
		}
		else
		{
			CODEC.WriteDW4(&locusAtom.userLocusEx.sidFlags);
			CODEC.WriteDW2(&locusAtom.userLocusEx.region);
			CODEC.WritePA1(&locusAtom.userLocusEx.utsAddr);
	...
	, const TSonorkDynString& 	sidText)
{
	ENCODE_REQ_SECTION_STARTS(SONORK_FUNCTION_SET_SID,sz,GUDPCV_SET_SID);
		if( sonorkServerVersion >= SONORK_EXTENDED_CODEC_VERSION_1 )
		{
			CODEC.Write(&locus);
		}
		else
		{
			CODEC.WriteDW4(&locus.userLocusEx.sidFlags);
			CODEC.WriteDW2(&locus.userLocusEx.region);
			CODEC.WritePA1(&locus.userLocusEx.utsAddr);
		}
		CODEC.Write(&sidText);
	ENCODE_SECTION_ENDS;
}
...
bool
 TSonorkDataPacket::D_SetSid_A(UINT size , TSonorkLocusAtom& locus)
{
	DECODE_AKN_SECTION_STARTS(size);
		...
		{
			locus.Clear();
			CODEC.ReadDW4(&locus.userLocusEx.sidFlags);
			CODEC.ReadDW2(&locus.userLocusEx.region);
			CODEC.ReadPA1(&locus.userLocusEx.utsAddr);
			locus.userLocusEx.hostAddr.Set( locus.userLocusEx.utsAddr );
...
#if defined(SONORK_SERVER_BUILD)
bool
 TSonorkDataPacket::D_SetUserData_R(UINT sz
	, DWORD sidVersion
	, TSonorkUserDataNotes&	UDN)
{
	DECODE_REQ_SECTION_STARTS(sz);
...
UINT
  TSonorkDataPacket::E_SetUserData_A(UINT sz
	, const TSonorkSerial& serial)
{
	ENCODE_NV_AKN_SECTION_STARTS(SONORK_FUNCTION_SET_USER_DATA,sz,GUDPCV_NONE);
...
// Old Sid event
UINT
 TSonorkDataPacket::E_OldSid_E(UINT sz
	, UINT items
	, TSonorkOldSidNotification*list)
{
   TSonorkOldSidNotification *ptr;
   UINT  			i;
   ENCODE_NV_EVT_SECTION_STARTS(SONORK_SERVER_CLIENT_EVENT_OLD_SID_NOTIFICATION,sz,GUDPCV_NONE);
   
	CODEC.WriteDW((DWORD)items);
	for(i=0,ptr=list;i<items;i++,ptr++)
	{
		CODEC.WriteDWN( (DWORD*)&ptr->locus.userId , 4 );
		if(!ptr->locus.sid.IsZero())
		{
			CODEC.WriteDW4(&ptr->locus.sidFlags);
			CODEC.WritePA1(&ptr->locus.physAddr);
		}
		CODEC.WriteDW2(&ptr->locus.region);
		CODEC.Skip( sizeof(ptr->locus.reserved) );
		CODEC.WriteDW2(&ptr->userSerial);
...
// New Sid event
UINT
 TSonorkDataPacket::E_Sid_E(UINT sz
	, DWORD 		sidVersion
 	, const TSonorkSidNotification& N)
{
...
UINT
 TSonorkDataPacket::E_NetworkConfig_E(UINT sz
		, DWORD sidVersion
		, DWORD serviceCount
		, const TSonorkServiceLocus*	servicePtr
		, DWORD 			sourceNetworkId
		, const TSonorkQueue*		networkQueue
		, DWORD 			maxNetworks
		)

{
	...
	ENCODE_EVT_SECTION_STARTS(SONORK_SERVER_CLIENT_EVENT_NETWORK_CONFIG,sz,GUDPCV_NONE);

	CODEC.WriteDW(serviceCount);
	for(itemNo=0;itemNo<serviceCount;itemNo++,servicePtr++)
	{
		CODEC.Write( servicePtr );
	}
	if(CODEC.AddDW(&networkCount))
	{
		itemNo=0;
		if( networkQueue )
		{
			networkQueue->BeginEnum(I);
			while((MAP=(const TSonorkP2Mapping*)networkQueue->w_EnumNext(I))!=NULL)
			{
				if(MAP->header.netId.v[0] == sourceNetworkId )
				{
					CODEC.Write(MAP);
					if(!CODEC.CodecOk())
						break;
					if(++itemNo==maxNetworks)
						break;
				}
			}
			networkQueue->EndEnum(I);
		}
		*networkCount=itemNo;
	}
	ENCODE_SECTION_ENDS;
...
bool
 TSonorkDataPacket::D_Sid_E(UINT sz,TSonorkSidNotificationQueue* queue)
{
	DWORD items;
	TSonorkSidNotification*	N;
	DECODE_EVT_SECTION_STARTS(sz);
		...
		while(CODEC.CodecOk() && items-- )
		{
			SONORK_MEM_NEW( N = new TSonorkSidNotification );
			CODEC.Read(N);
			if( CODEC.CodecOk() )
				queue->Add(N);
			else
...
// Services Event
bool
 TSonorkDataPacket::D_NetworkConfig_E(
			UINT sz
			,TSonorkQueue& services
			,TSonorkQueue& networks)
{
	DWORD itemCount;
	union
	{
	...
	CODEC.ReadDW(&itemCount);
	itemCount&=0xfff;
	while( services.Items()<itemCount && CODEC.CodecOk() )
	{
		SONORK_MEM_NEW(D.serviceLocus = new TSonorkServiceLocus);
		CODEC.Read(D.serviceLocus);
		if( CODEC.CodecOk() )
		{
			services.w_Add(D.serviceLocus);
		}
		else
		{
	...
	CODEC.ReadDW(&itemCount);
	itemCount&=0xfff;
	while( networks.Items()<itemCount && CODEC.CodecOk() )
	{
		SONORK_MEM_NEW(D.p2Network = new TSonorkP2Mapping);
		CODEC.Read(D.p2Network);
		if( CODEC.CodecOk() )
		{
			networks.w_Add(D.p2Network);
		}
		else
		{
...
UINT
 TSonorkDataPacket::E_MsgNotify_E(
	  UINT 				sz
	, UINT 				items
	, TSonorkOldMsgNotification*	ptr)
{
   ENCODE_NV_EVT_SECTION_STARTS(SONORK_SERVER_CLIENT_EVENT_OLD_MSG_NOTIFICATION
	,sz
	,GUDPCV_NONE);
	CODEC.WriteDW((DWORD)items);
	while(items--)
...
UINT  TSonorkDataPacket::E_MsgDelivery_E(UINT sz
	, DWORD 	sidVersion
	, TSonorkObjId* msg_id
	, TSonorkMsg*	msg)
{
...
bool
 TSonorkDataPacket::D_PutMsg_R(UINT sz
	, DWORD 		sidVersion
...
bool
 TSonorkDataPacket::D_PutMsg_From_R(UINT sz
	...
	, TSonorkMsg&			msg)
{
	return D_PutMsg_R(sz,sidVersion,target,msg);
}
...
{
	ENCODE_NV_AKN_SECTION_STARTS(f,sz,GUDPCV_NONE);
		CODEC.WriteDW2(&msg_id);
	ENCODE_SECTION_ENDS;
}
...
bool   TSonorkDataPacket::D_DelMsg_R(UINT sz
	, DWORD& count
	, TSonorkObjId *msg_id)
{
	UINT i;
...
bool
 TSonorkDataPacket::D_SetGroup_R2(UINT sz
	, DWORD 	sidVersion
	, TSonorkGroup&	UG)
{
	DECODE_REQ_SECTION_STARTS(sz);
...
UINT  TSonorkDataPacket::E_Error_A(UINT sz
	, SONORK_FUNCTION 	function
	, const TSonorkError&ERR)
{
	InitHeader(function,SONORK_PACKET_TYPE_AKN);
	TSonorkPacketWriter  CODEC(DataPtr(),sz,0);
	E_Error(CODEC,ERR);
	SetNoDataMark();
	return CODEC.CodecOk()?sizeof(TSonorkDataPacket)+CODEC.Size():0;
}
bool  TSonorkDataPacket::D_Error_A(UINT sz
	, TSonorkError&ERR)
{
	if( IsErrorMarkSet() )
	{
		TSonorkPacketReader CODEC(DataPtr(),sz,0);
		CODEC.Read(&ERR);
		if( !CODEC.CodecOk())
		{
...
UINT TSonorkDataPacket::E_ReqAuth_A(
	...
	,const TSonorkId&		user_id)
{
	ENCODE_NV_AKN_SECTION_STARTS(SONORK_FUNCTION_REQ_AUTH,sz,GUDPCV_NONE);
		CODEC.WriteDW2(&user_id);
	ENCODE_SECTION_ENDS;
}
...
{
	ENCODE_NV_AKN_SECTION_STARTS(SONORK_FUNCTION_AKN_AUTH,sz,GUDPCV_NONE);
		CODEC.WriteDW2(&user_id);
		CODEC.WriteAU2(&auth);
	ENCODE_SECTION_ENDS;
}
...
UINT TSonorkDataPacket::E_SetAuth_A(
	...
	,const TSonorkAuth2&	auth)
{
	ENCODE_NV_AKN_SECTION_STARTS(SONORK_FUNCTION_SET_AUTH,sz,GUDPCV_NONE);
		CODEC.WriteDW2(&user_id);
		CODEC.WriteAU2(&auth);
	ENCODE_SECTION_ENDS;
}
...
UINT
 TSonorkDataPacket::E_DelAuth_A(
	...
	,const TSonorkId&		user_id)
{
	ENCODE_NV_AKN_SECTION_STARTS(SONORK_FUNCTION_DEL_AUTH,sz,GUDPCV_NONE);
		CODEC.WriteDW2(&user_id);
	ENCODE_SECTION_ENDS;
}
...
TSonorkDataPacket*
 TSonorkDataPacket::Alloc(DWORD dataSize)
{
	return SONORK_MEM_ALLOC(TSonorkDataPacket,sizeof(TSonorkDataPacket) + sizeof(DWORD) + dataSize  );
}
