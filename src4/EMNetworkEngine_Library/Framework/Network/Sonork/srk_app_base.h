...
private:
	...
	void	PSE_UserSid(const TSonorkLocusAtom*
			,const TSonorkSerial*
			,TSonorkDynString*);
	...
	virtual void
		OnSonorkUserSid(TSonorkExtUserData*
			,const TSonorkLocusAtom* old_locus){}
	virtual void
	...
	virtual void
		OnSonorkCtrlMsg(const TSonorkLocus1*,const TSonorkCtrlMsg*,TSonorkDynData*){}
...
// Global profile
	...
	DWORD
		...
		{ return app.version.VersionNumber(); }
...
// Information
	...
	const TSonorkUserAddr&
		ProfileUserAddr() const
		{ return profile_user.addr; }
		
	const TSonorkSid&
		ProfileSid() 	const
		{ return profile_user.addr.header.sid;}
...
// Misc
	...
	bool
		GetServerService(SONORK_SERVICE_TYPE serviceType
			, DWORD serverNo
			, TSonorkServiceLocus& serviceLocus)
		{ return sonork.GetServerService(serviceType,serverNo,serviceLocus); }
		
	const	TSonorkP2Mapping*
		GetNetworkMapping(DWORD networkId)
		{ return sonork.GetNetworkMapping(networkId); }
