...
DWORD
 TSonorkEmailAccount::CODEC_DataSize(DWORD )	const
{
	DWORD rv;
...
DWORD
 TSonorkEmailExcept::CODEC_DataSize(DWORD )	const
{
	return sizeof(DWORD)*3
			+ sizeof(header)
			+ ::CODEC_Size(&name)
			+ queue.SumAtomsCodecSize(SONORK_NULL_CODEC_VERSION);
}
...
DWORD
 TSonorkEmailExceptEntry::CODEC_DataSize(DWORD)	const
{
