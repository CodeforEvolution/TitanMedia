...
eo << "EMGUIUtilities: EM_SETTING_UPDATED" << ef;
		if(EMSettingsRepository::Instance() -> SettingExists(SETTING_EDIT_MODE))
...
//;//cout_commented_out_4_release << "opString: " << opString << endl;
		if((opSnappingMode != NULL && vTempo != -1 && opSignature != NULL) && (opSnappingMode != m_opSnappingMode || vTempo != m_vTempo || opSignature != m_opSignature))
		{
