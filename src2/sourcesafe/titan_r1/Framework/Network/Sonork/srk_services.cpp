...
DWORD
 TSonorkAuthsAtom2::CODEC_DataSize(DWORD codecVersion) const
{
	return  sizeof(DWORD)
		+	::CODEC_Size(&auth)
		+  	user_data.CODEC_Size(codecVersion);
}
void
...
DWORD
 TSonorkDataServerLoginReq::CODEC_DataSize(DWORD )	const
{
...
DWORD
 TSonorkFileInfo::CODEC_DataSize(DWORD) const
{
	return		sizeof(DWORD)
			+ sizeof(locus)
			+ sizeof(attr)
...
DWORD
  TSonorkDataServerPutFileTargetResult::CODEC_DataSize(DWORD codecVersion) const
{
	return sizeof(DWORD) + sizeof(header) + ERR.CODEC_Size(codecVersion);
}
void
...
DWORD
 TSonorkDataServerNewPutFileReq::CODEC_DataSize(DWORD codecVersion) const
{
	return sizeof(DWORD)
		+ sizeof(header)
		+ file_info.CODEC_Size(codecVersion)
		+ target_atom.CODEC_Size(codecVersion)
		+ crypt_info.CODEC_Size(codecVersion);
}
...
DWORD
 TSonorkDataServerOldPutFileReq::CODEC_DataSize(DWORD codecVersion) const
{
	return sizeof(DWORD) + sizeof(header) + file_info.CODEC_Size(codecVersion);
}
...
DWORD
 TSonorkDataServerNewGetFileReq::CODEC_DataSize(DWORD codecVersion) const
{
	return sizeof(DWORD)
		+ sizeof( header )
		+ sizeof( locus )
...
DWORD
 TSonorkDataServerOldGetFileReq::CODEC_DataSize(DWORD ) const
{
...
DWORD
 TSrkIpcPacket::E_Req(DWORD D_size, UINT function, const TSonorkCodecAtom*A,DWORD codecVersion)
{
	header.ipc_function 	= function;
	header.ipc_version  	= 0;
	if(A)
	{
		if(A->CODEC_WriteMem(DataPtr(), D_size , codecVersion)!=SONORK_RESULT_OK )
		{
...
bool
 TSrkIpcPacket::D_Req( DWORD P_size, TSonorkCodecAtom*A , DWORD codecVersion)
{
	...
	else
	{
		...
		if(A->CODEC_ReadMem( DataPtr(), P_size, codecVersion)==SONORK_RESULT_OK)
			return true;
	}
	return false;
...
DWORD
 TSrkIpcPacket::E_Akn(DWORD D_size, UINT function, const TSonorkCodecAtom*A, DWORD codecVersion)
{
	header.ipc_function 	= function;
	header.ipc_version  	= 0;
	if(A)
...
bool
 TSrkIpcPacket::D_Akn(DWORD P_size, TSonorkCodecAtom*A, DWORD codecVersion)
{
	if( (header.ipc_flags & SONORK_IPC_PACKET_F_INVALID) ||  P_size < sizeof(header))
		return false;
 ...
 TSrkIpcPacket::E_Error(DWORD D_size, UINT function, const TSonorkError& ERR)
{
	...
	ERR.CODEC_WriteMem(DataPtr(),D_size,SONORK_V15_COMPATIBLE_CODEC_VERSION);
	return D_size+sizeof(header);
}
bool
 TSrkIpcPacket::D_Error(DWORD P_size, TSonorkError&ERR)
{
	...
	if( !(header.ipc_flags & SONORK_IPC_PACKET_F_ERROR ))
		return false;
	P_size-=sizeof(header);
	return ERR.CODEC_ReadMem( DataPtr(), P_size,SONORK_V15_COMPATIBLE_CODEC_VERSION) == SONORK_RESULT_OK;
