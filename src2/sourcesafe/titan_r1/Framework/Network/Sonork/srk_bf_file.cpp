 ...
 TSonorkBfFile::Read(TSonorkBfBlock* dir_block, const char *name, TSonorkCodecAtom*S)
{
	...
	{
		DWORD sz=B->DataSize();
		last_result=S->CODEC_ReadMem(B->Data(),sz,SONORK_CURRENT_CODEC_VERSION);
		SONORK_MEM_DELETE(B);
	}
	else
		S->CODEC_Clear();
 ...
 TSonorkBfFile::Write(TSonorkBfBlock* dir_block, const char*name, const TSonorkCodecAtom*S)
{
	...
	if( B != NULL )
	{
		DWORD sz=S->CODEC_Size(SONORK_CURRENT_CODEC_VERSION);
		B->SetData(NULL,sz);
		if((last_result=S->CODEC_WriteMem(B->Data(),sz,SONORK_CURRENT_CODEC_VERSION))==SONORK_RESULT_OK)
		{
