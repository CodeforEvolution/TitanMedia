...
DWORD
 TSonorkCodecAtom::CODEC_Size(DWORD version) const
{
	return CODEC_DataSize(version)+sizeof(SONORK_CODEC_DESCRIPTOR);
}
SONORK_RESULT
 TSonorkCodecAtom::CODEC_Copy(const TSonorkCodecAtom*O)
{
	BYTE*		buffer;
	DWORD		codec_size;
	SONORK_RESULT   result;
	codec_size = O->CODEC_Size(SONORK_CURRENT_CODEC_VERSION);
	buffer	= SONORK_MEM_ALLOC(BYTE,codec_size);
	result	= O->CODEC_WriteMem(buffer , codec_size , SONORK_CURRENT_CODEC_VERSION);
	if(result==SONORK_RESULT_OK)
		result = this->CODEC_ReadMem(buffer,codec_size,SONORK_CURRENT_CODEC_VERSION);
...
char *TSonorkCodecAtom::CODEC_GetTextView(char*pTgt,DWORD size) const
{
	if(size>32)
	{
		sprintf(pTgt,"<ATOM T:%u S:%u>"
			,CODEC_DataType()
			,CODEC_Size(SONORK_CURRENT_CODEC_VERSION));
	}
	else
		*pTgt=0;
	return pTgt;
...
SONORK_RESULT
 TSonorkCodecAtom::CODEC_WriteMem( BYTE*ptr , DWORD& size , DWORD version  ) const
{
	SONORK_RESULT	result;
	...
	else
	{
		...
		if( CODEC_DataSize(version) > 0 )
		{
			TSonorkCodecWriter
				CODEC(ptr,size-sizeof(SONORK_CODEC_DESCRIPTOR),data_type,version);

			CODEC_WriteDataMem(CODEC);

			if( (result =CODEC.Result()) == SONORK_RESULT_OK)
			{
				*e_ptr=SONORK_DWORD( EncodeDescriptor(data_type,CODEC.Size()) );
		...
		else
		{
			...
			*e_ptr=SONORK_DWORD( EncodeDescriptor(data_type,0) );
...
SONORK_RESULT
	TSonorkCodecAtom::CODEC_ReadMemNoSize(const BYTE*ptr,DWORD size, DWORD version)
{
	return CODEC_ReadMem(ptr,size,version);
}

SONORK_RESULT
	TSonorkCodecAtom::CODEC_ReadMem(const BYTE*ptr,DWORD& size, DWORD version)
{
	SONORK_RESULT	result;
	CODEC_Clear();
	if( size < sizeof(SONORK_CODEC_DESCRIPTOR) )
	...
	else
	{
		...
		DecodeDescriptor(descriptor,data_type,data_size);
		if(data_size)
		{
			...
			{
				TSonorkCodecReader CODEC(ptr,data_size, data_type , version);
...
{
	data=p_data;
	DecodeDescriptor(D,data_type,data_size);
}
...
SONORK_CODEC_DESCRIPTOR
 TSonorkCodecAtom::EncodeDescriptor(SONORK_ATOM_TYPE type,DWORD sz)
{
	SONORK_CODEC_DESCRIPTOR descriptor;
	descriptor=type;
...
void
 TSonorkCodecAtom::DecodeDescriptor(SONORK_CODEC_DESCRIPTOR descriptor,SONORK_ATOM_TYPE&type,DWORD& sz)
{
...
SONORK_RESULT
	TSonorkCodecAtom::CODEC_Read(const TSonorkDynData*dyn_data, DWORD version)
{
	CODEC_Clear();
...
SONORK_RESULT
 TSonorkCodecAtom::CODEC_Write(TSonorkDynData*dyn_data,DWORD version) const
{
	BYTE				*ptr;
	SONORK_CODEC_DESCRIPTOR	*e_ptr;
	SONORK_ATOM_TYPE		data_type;
	...
	data_type=CODEC_DataType();
	data_size=CODEC_DataSize(version);
	...
	if(data_size > 0)
	{
		TSonorkCodecWriter CODEC(ptr,data_size,data_type,version);
		CODEC_WriteDataMem(CODEC);
