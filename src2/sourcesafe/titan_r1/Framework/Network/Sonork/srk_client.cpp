...
TSonorkClient::TSonorkClient(TSonorkEventHandler*handler , void  *tag)
{
	...
	server.physAddr.Clear();

	control.userId.Clear();
	control.sid.Clear();
	control.sidFlags.Clear();
	control.hostAddr.Clear();
 ...
 TSonorkClient::Connect(TSonorkError&ERR,TSonorkPhysAddr& server_phys_addr)
{
	...
	{
		server.physAddr.Set(server_phys_addr);
...
{
	int err_code;
	err_code=UDP_NetIO().Connect( server.physAddr
		, (1024*16)+512
		, (1024*24)+512);
...
SONORK_RESULT TSonorkClient::TCP_Connect(TSonorkError&ERR)
{
	...
	TCP_NetIO().SetSocksInfo(control.socks_info);

	err_code=TCP_NetIO().Connect(server.physAddr);
	if(!err_code)
	{
		ERR.SetOk();
	}
 ...
 TSonorkClient::TCP_ConnectionLost()
{
	...
	ERR.SetSys(
		...
		,GSS_NETCXLOST,SONORK_MODULE_LINE);

	SetStatus(SONORK_NETIO_STATUS_DISCONNECTED
		,&ERR
		,SONORK_CLIENT_STATUS_CHANGE_UNEXPECTED
		,"TCP_ConnectionLost()");
}
 ...
 TSonorkClient::UDP_Recv(BYTE*raw_data, DWORD recv_size)
{
	...
	if( hdr->Type() == SONORK_NETIO_HFLAG_TYPE_PACKET )
	{
		...
		if( entry->SID() == control.sid.SessionId() )
		{
	...
	if(hdr->Type()==SONORK_NETIO_HFLAG_TYPE_LOGOUT)
	{
		...
		if( hdr->SID()==control.sid.SessionId() )
		{
			TSonorkError ERR;
			...
			SetStatus(SONORK_NETIO_STATUS_DISCONNECTED
				,&ERR
				,SONORK_CLIENT_STATUS_CHANGE_UNEXPECTED
				|SONORK_CLIENT_STATUS_CHANGE_CX_KILLED
 ...
 TSonorkClient::TCP_Send()
{
	if(control.requests_queue.Items())
	{
		if( control.requests_table.CurEntries() < control.requests_table.MaxEntries() )
		{
			...
			request->idleClock.Set( TimeSlotClock() );
	...
	else
	{
		...
		if( entry_no > PckTimeoutMsecs() )
		{
			...
			SetStatus(SONORK_NETIO_STATUS_DISCONNECTED
				,&ERR
				,SONORK_CLIENT_STATUS_CHANGE_UNEXPECTED
				|SONORK_CLIENT_STATUS_CHANGE_BY_TIMEOUT
				|SONORK_CLIENT_STATUS_CHANGE_CX_LOST
 ...
 TSonorkClient::ProcessLoginAkn(BYTE *data,DWORD data_size)
{
	...
	DWORD			   	flags=SONORK_CLIENT_STATUS_CHANGE_UNEXPECTED;

	ERR.SetResult(AKN.CODEC_ReadMem(data,data_size,SONORK_NULL_CODEC_VERSION));
	if( ERR.Result() != SONORK_RESULT_OK )
	{
		ERR.SetSysExtInfo(GSS_BADCODEC,SONORK_MODULE_LINE);
		flags|=SONORK_CLIENT_STATUS_CHANGE_BAD_PROTOCOL;
	}
	else
	if(AKN.header.result==SONORK_RESULT_OK)
	{
		...
		else
		{
			...
			else
			{
				...
				control.sid.Set( SI->sid );
				control.sidFlags.Set( SI->sidFlags );
				control.hostAddr.Set( SI->physAddr );
				...
				sonork_printf("SERVER.VERSION=%x",sonorkServerVersion);
	...
	{
		flags|=SONORK_CLIENT_STATUS_CHANGE_CX_DENIED;
			...
			if(CX->signature==SONORK_SERVER_SIGNATURE)
			{
				...
				else
				{
					...
					SetStatus(SONORK_NETIO_STATUS_DISCONNECTED
						,&aknERR
						,SONORK_CLIENT_STATUS_CHANGE_UNEXPECTED
						|SONORK_CLIENT_STATUS_CHANGE_CX_DENIED
						,"TCP_ProcessInPacket(HFLAG_TYPE_CONNECT)");
				}
			}
		}
			...
			)
{
	...
	else
	{
		...
		taskFunction=data_packet->Function();
		taskSubFunction=data_packet->SubFunction();

		if(++control.cur_task_id>SONORK_UDP_PACKET_UID_MAX_VALUE)
			control.cur_task_id=1;

		new_task_id=control.cur_task_id;
		data_packet->ReqTaskId().v[TSonorkTaskId::V_CLIENT]=new_task_id;
		...
		if(enc_result==SONORK_RESULT_OK)
		{
			...
			else
			{
				...
				tcp_packet=TSonorkTcpPacket::Alloc(DATA_size);
    ...
    else
    {
        ...
        if( request != NULL )
        {
            if(Protocol()==SONORK_NETIO_PROTOCOL_UDP)
            {
                ...
                {
                    if( UDP_OutPacket()->UID() == request->Id() )
                    {
	...
	, SONORK_DWORD2*p_tag)
{
    ...
    if( request != NULL )
    {
        ...
        control.requests_table.DelEntry( request->Id() );
    }
    else
    {
...
void TSonorkClient::RequestEnd(TSonorkClientRequest*request, TSonorkError&ERR)
{
	...
	control.requests_table.DelEntry(requestId);
	if( !request->Canceled() )
	{
...
const	TSonorkP2Mapping*
 TSonorkClient::GetNetworkMapping(DWORD networkId)
{
	union
	{
		void*			ptr;
		TSonorkP2Mapping*	p2Network;
	}D;
	TSonorkListIterator I;
	TSonorkP2Mapping*	p2Network=NULL;

	control.mutex.Lock( __FILE__ , SONORK_MODULE_LINE );
	server.networks.BeginEnum(I);
	while((D.ptr=server.networks.w_EnumNext(I)) != NULL )
	{
		if(D.p2Network->TargetNetId() == networkId)
		{
			p2Network=D.p2Network;
			break;
		}
	}
	server.networks.EndEnum(I);
	control.mutex.Unlock();
...
bool
 TSonorkClient::GetServerService(
	...
	, TSonorkServiceLocus& 	serviceLocus)
{
	...
	control.mutex.Lock( __FILE__ , SONORK_MODULE_LINE );
	server.services.BeginEnum(I);
	while((D.ptr=server.services.w_EnumNext(I)) != NULL )
	{
		if(D.serviceLocus->ServiceType() != serviceType )
			continue;
		if(serverInstance!=0)
			if( D.serviceLocus->ServiceInstance() != serverInstance)
				continue;
		found=true;
		serviceLocus.Set(*D.serviceLocus);
		break;
	}
	server.services.EndEnum(I);
	control.mutex.Unlock();
 ...
 TSonorkClient::SetStatus(SONORK_NETIO_STATUS new_status,TSonorkError* statusERR,DWORD sFlags, SONORK_C_CSTR pCaller )
{
	...
	{
		void*			ptr;
		TSonorkClientRequest*	clientRequest;
		TSonorkServiceLocus*	serviceLocus;
		TSonorkP2Mapping*	p2Network;
	}D;
	...
	if(new_status==SONORK_NETIO_STATUS_DISCONNECTED)
	{
		...
		if(old_status>=SONORK_NETIO_STATUS_CONNECTED)
			sFlags|=SONORK_CLIENT_STATUS_CHANGE_CX_LOST;

		requestERR.SetSys(SONORK_RESULT_NETWORK_ERROR,GSS_NETCXLOST,SONORK_MODULE_LINE);

		control.requests_table.InitEnum(&index);

		while((D.clientRequest=control.requests_table.EnumNext(&index))!=NULL)
		{
			RequestEnd(D.clientRequest,requestERR);
		}

		while((D.clientRequest=control.requests_queue.RemoveFirst())!=NULL)
		{
			RequestEnd(D.clientRequest,requestERR);
		}

		while((D.ptr=server.services.w_RemoveFirst()) != NULL )
		{
			SONORK_MEM_DELETE( D.serviceLocus );
		}
		while((D.ptr=server.networks.w_RemoveFirst()) != NULL )
		{
			SONORK_MEM_DELETE( D.p2Network );
		}
 ...
 TSonorkClient::LoadConnectRequest(BYTE*data,DWORD data_size)
{
	...
	REQ.CODEC_WriteMem( data , data_size , SONORK_NULL_CODEC_VERSION); // Modifies data_size
	return data_size;
}
...
void TSonorkClient::UDP_SendDisconnectRequest()
{
	...
	UDP_SendBuffer().header.sid  =control.sid.SessionId();
 ...
 TSonorkClient::UDP_SendEntry(DWORD entry_no, bool /*is_resending*/)
{
	...
	UDP_SendBuffer().entry.sid=SONORK_DWORD(control.sid.SessionId());
	...
	switch(P->Function())
	{
		case  SONORK_SERVER_CLIENT_EVENT_OLD_MSG_NOTIFICATION:
		{
			...
			if(P->D_OldMsgNotify_E(P_size,&list))
			{
				...
				{
					RP=TSonorkDataPacket::Alloc(RP_alloc);
		...
		{
			sonork_printf("OLD_SID_NOTIFICATION");
			/*
		...
		case SONORK_SERVER_CLIENT_EVENT_SID_NOTIFICATION:
		{
			...
			if(P->D_Sid_E(P_size,&queue))
			{
				...
				{
					app.event.type	 = SONORK_CLIENT_EVENT_USER_SID;
					app.event.user_id.Set(N->locus.userLocusEx.userId);
					app.event.data.sid.locus = &N->locus;
					app.event.data.sid.userSerial.Set(N->header.userSerial);
		...
		case  SONORK_SERVER_CLIENT_EVENT_NETWORK_CONFIG:
		{
			union
			{
				void*			ptr;
			...
			char tmp1[48],tmp2[48];

			P->D_NetworkConfig_E( P_size
				, server.services
				, server.networks);

			server.networks.BeginEnum(index);
			while((D.ptr = server.networks.w_EnumNext(index)) != NULL )
			{
				sonork_printf("Net {%u->%u} %x %s:%u %s:%u"
					, D.p2Network->header.netId.v[0]
					, D.p2Network->header.netId.v[1]
					, D.p2Network->header.flags
					, D.p2Network->mapperHost.CStr()
					, D.p2Network->header.mapperPort
					, D.p2Network->socksHost.CStr()
					, D.p2Network->header.socksPort
					);

			}
			server.networks.EndEnum(index);

			server.services.BeginEnum(index);
			while((D.ptr = server.services.w_EnumNext(index)) != NULL )
			{
				sonork_printf("%u UTS=%s HOST=%s"
					, D.serviceLocus->ServiceType()
					, D.serviceLocus->locus.userLocusEx.utsAddr.GetStr(tmp1)
					, D.serviceLocus->locus.userLocusEx.hostAddr.GetStr(tmp2)
					);

			}
			server.services.EndEnum(index);
			sonork_printf("Networks: %u , Services: %u"
				, server.networks.Items()
...
{
	TSonorkClientRequest   		*request;
	TSonorkTaskId			aknTaskId;
	aknTaskId.Set(DATA_packet->AknTaskId());

	request=control.requests_table.GetEntry(aknTaskId.Client());
	if( request )
	{
		TSonorkError ERR;
		...
		if(aknTaskId.Server()==SONORK_INVALID_TASK_ID || request->Canceled())
			RequestEnd(request , ERR);
		control.active_request=NULL;
	}
	else
		sonork_printf("O_PACKET not found for task %u",aknTaskId.Client());
...
TSonorkClientRequest	*
 TSonorkClientRequestTable::GetEntry(DWORD requestId) const
{
	UINT entryNo;
	TSonorkClientRequest**dPtr;
	for(entryNo=0,dPtr=table_ptr;entryNo<table_size;entryNo++,dPtr++)
		if(*dPtr)
		if((*dPtr)->Id()==requestId)
			return *dPtr;
	return NULL;
}
...
bool
 TSonorkClientRequestTable::DelEntry(DWORD requestId)
 {
	DWORD entryNo;
	TSonorkClientRequest**dPtr;
	for(entryNo=0,dPtr=table_ptr;entryNo<table_size;entryNo++,dPtr++)
 ...
 TSonorkClientRequestTable::EnumNext(UINT*i) const
{
	...
	if(entryNo<table_size)
	{
		for(dPtr=table_ptr+entryNo;entryNo<table_size;entryNo++,dPtr++)
		{
			if(*dPtr)
			{
				*i=entryNo+1;
	...
	TSonorkClient::CreateNetIO(TSonorkError&ERR)
{
	...
	ERR.SetOk();
	if(server.physAddr.Type() == SONORK_PHYS_ADDR_UDP_1)
	{
	...
	if(server.physAddr.Type() == SONORK_PHYS_ADDR_TCP_1)
	{
...
TSonorkClientRequest::TSonorkClientRequest(
	 DWORD						pId
	,SONORK_FUNCTION 			pFunction
	,DWORD						pSubFunction
	,lpfnSonorkRequestHandler	pCbProc
	,void*						pCbParam
...
TSonorkClientRequest* 
 TSonorkClientRequestQueue::Peek(DWORD requestId)
{
...
TSonorkClientRequest* 
 TSonorkClientRequestQueue::Remove(DWORD requestId)
{
	TSonorkClientRequest* R=Peek(requestId);
    if(R)w_Remove(R);
    return R;
}
...
void TSonorkClient::DoMonitor()
{
	...
	if( control.requests_table.CurEntries() )
	{
		...
		maxReqSecs =   ReqTimeoutMsecs() / 1000;
		control.requests_table.InitEnum(&index);
		while((request=control.requests_table.EnumNext(&index))!=NULL)
		{
			to=TimeSlotClock().IntervalSecsAfter( request->idleClock);
			if(to>(maxReqSecs/2)+1)
			{
				sonork_printf("TSonorkClient::DoMonitor:TID=%x TO=%u/%u"
					,request->Id()
					,to,maxReqSecs);
			}
			if( to > maxReqSecs )
...
void TSonorkClient::PingServer( UINT )
{
	...
	UINT P_size;
	TSonorkDataPacket *P;
	P=TSonorkDataPacket::Alloc(64);
	P_size=P->E_Ping_R();
	SelfRequest(ERR,P,P_size);
}
// -----------------------------------------------------------------------------
int
 TSonorkClient::GetLocalAddr(TSonorkPhysAddr& physAddr)
{
	if(Status() < SONORK_NETIO_STATUS_CONNECTED)
		return -1;
	if( Protocol() == SONORK_NETIO_PROTOCOL_UDP )
	{
		return UDP_NetIO().GetSocketAddr(physAddr);
	}
	else
	{
		return TCP_NetIO().GetSocketAddr(physAddr);
	}
...
bool
 TSonorkClient::IsUsingSocks() const
{
	if(Status() > SONORK_NETIO_STATUS_DISCONNECTED)
	{
		if(Protocol()==SONORK_NETIO_PROTOCOL_TCP)
			...
			,TSonorkTrackerDataQueue& queue)
{
	...
	{
		A_size = queue.SumAtomsCodecSize(sonorkServerVersion) + 32;
		while( A_size >= 65536 )
		{
		...
		P=TSonorkDataPacket::Alloc(A_size);
...
void
 TSonorkClient::SyncSid(TSonorkError&ERR
			...
			, const TSonorkDynString&	sidText)
{
	UINT 	P_size,A_size;
	TSonorkDataPacket *P;
	control.mutex.Lock( __FILE__ , SONORK_MODULE_LINE );
	if(MayStartGlobalTask(ERR))
	{
		locus.userLocusEx.userId.Set( control.userId );
		locus.userLocusEx.sid.Set( control.sid );
		locus.userLocusEx.hostAddr.Set( control.hostAddr );
		A_size=locus.CODEC_Size(sonorkServerVersion) + sidText.CODEC_Size(sonorkServerVersion)+64;
		P=TSonorkDataPacket::Alloc(A_size);
		P_size=P->E_SetSid_R(A_size, locus, sidText);
		SelfRequest(ERR,P,P_size);
		if( ERR.Result() == SONORK_RESULT_OK )
 ...
 TSonorkClient::SelfHandleDataPacket(TSonorkClientRequestEvent*E)
{
	...
	switch( P->Function() )
	{
		...
		{
			TSonorkLocusAtom	locus;
			if( P->D_SetSid_A( P_size, locus) )
			{
				control.sidFlags.Set(locus.userLocusEx.sidFlags);
				control.hostAddr.Set(locus.userLocusEx.hostAddr);
			}
		}
		break;
		...
		if( P->SubFunction() == SONORK_SUBFUNC_USER_LIST_USERS)
		{
			...
			{
				if(PTR.udn->userData.GetUserInfoLevel()
					>= SONORK_USER_INFO_LEVEL_1)
				{
					app.event.type		   			= SONORK_CLIENT_EVENT_USER_ADD;
					app.event.user_id.Set( PTR.udn->userData.userId );
					app.event.user_data				= &PTR.udn->userData;
					app.event.data.userAddSet.flags	= SONORK_MSG_SF_SELF_TRIGGERED;
					app.event.data.userAddSet.auth	= &PTR.udn->auth;
					app.event.data.userAddSet.notes	= &PTR.udn->notes;
					InvokeEventHandler();
				}
				else
					sonork_printf("------USER_LIST_PACKET: USER(%u.%u) LEVEL=%u--------"
						,PTR.udn->userData.userId.v[0]
						,PTR.udn->userData.userId.v[1]
						,PTR.udn->userData.GetUserInfoLevel()
						);
		...
		if( P->SubFunction() == SONORK_SUBFUNC_NONE )
		{
			...
			{
				if(ND.userData.GetUserInfoLevel()>=SONORK_USER_INFO_LEVEL_1)
				{
					app.event.type			= SONORK_CLIENT_EVENT_USER_SET;
					app.event.user_id.Set(ND.userData.userId);
					app.event.user_data		= &ND.userData;
					app.event.data.userAddSet.flags	= 0;
					app.event.data.userAddSet.notes	= &ND.notes;
					app.event.data.userAddSet.auth	= &ND.auth;
					InvokeEventHandler();
				}
				else
				{
 ...
 TSonorkClient::SelfHandleGroups(TSonorkDataPacket*P,UINT P_size)
{
	...
	if( P->D_UserGroups_A(P_size,queue) )
	{
		...
		{
			app.event.type	 			= SONORK_CLIENT_EVENT_GROUP_ADD;
...
{
	DWORD	sysMsgCodecVersion;

	if( 	msg->DataServiceId() 	!= SONORK_SERVICE_ID_SONORK
	||	msg->DataServiceType()	!= SONORK_SERVICE_TYPE_AUTHS)
		return false;

	sysMsgCodecVersion=msg->DataServiceHFlags()&SONORK_SERVICE_HF_SYS_MSG_VERSION;
	if(sysMsgCodecVersion < SONORK_SERVICE_HF_SYS_MSG_V1)
	{

		sysMsgCodecVersion=SONORK_V15_COMPATIBLE_CODEC_VERSION;
...

	{
		...
		sysMsgCodecVersion=SONORK_EXTENDED_CODEC_VERSION_1;
...
//    and AUTH_DEL message.
		case SONORK_AUTHS_ATOM_ADV:
			...
			//SONORK_Trace("TSonorkClient::ProcessSysMsg(AUTH_ADV)");
			{
				...
				if( auth_adv.CODEC_Read(msg->ExtDataPtr(),sysMsgCodecVersion) != SONORK_RESULT_OK)
				{
				...
				auth_req.userData.Set(auth_adv.user_data);
				auth_req.notes.Set(msg->Text().String());

				auth_req.header.requestorId.Set( msg->UserId() );
...
//    has also been deleted.
		case SONORK_AUTHS_ATOM_ADD:
			//SONORK_Trace("TSonorkClient::ProcessSysMsg(AUTH_ADD)");
			{
				...
				if( auth_add.CODEC_Read(msg->ExtDataPtr(),sysMsgCodecVersion) != SONORK_RESULT_OK )
				{
					// This should never happen unless the msg was
					// an invalid packet that could not be decoded.
					break;
				...
				else
				{
					...
					app.event.data.userAddSet.flags=msg->SysFlags();
					app.event.data.userAddSet.auth =&auth_add.auth;
					app.event.data.userAddSet.notes=&msg->text.string;
					app.event.pERR		=NULL;
					InvokeEventHandler();
		...
		,UINT			data_size)
{
	...
	else
	{
		TSonorkDataPacket	*P;
		UINT P_size, A_size;
		A_size=sizeof(TSonorkLocus1) + sizeof(TSonorkCtrlMsg) + data_size + 32;
...
SONORK_RESULT TSonorkClient::PutMsg(TSonorkError&ERR,const TSonorkLocus1*locus,TSonorkMsg* msg)
{
	control.mutex.Lock( __FILE__ , SONORK_MODULE_LINE );
