...
#define V15_DATA_ATOM_VERSION	1
 ...
 TSonorkExtUserList::Add(const TSonorkExtUserDataPtr newUD)
{
	...
	memcpy(&listUD->ctrlData
		,&newUD->ctrlData
		,sizeof(listUD->ctrlData));
	return listUD;
}
...
DWORD TSrkExtUserDataAtom::CODEC_DataSize(DWORD codecVersion) const
{
	return sizeof(DWORD)*8
		+ UD->CODEC_Size( codecVersion)
...
void
 TSrkExtUserDataAtom::CODEC_WriteDataMem( TSonorkCodecWriter& CODEC) const
{
	CODEC.WriteDW( CODEC.Version() );
	CODEC.WriteDW(UD->UserType());
	CODEC.WriteDW(sizeof(UD->ctrlData));
	CODEC.Skip(sizeof(DWORD)*5);
	CODEC.Write( UD );
	CODEC.Write( &UD->display_alias );
	CODEC.Write( &UD->sound.online );
	CODEC.Write( &UD->sound.message );
	CODEC.WriteDWN((DWORD*)&UD->ctrlData , SIZEOF_IN_DWORDS(UD->ctrlData) );
}


void TSrkExtUserDataAtom::CODEC_ReadDataMem( TSonorkCodecReader& CODEC)
{
	DWORD	tmpVersion,storedVersion,type,ctrlDataCodecFlags;

	CODEC.ReadDW(&storedVersion);
	CODEC.ReadDW(&type);
	CODEC.ReadDW(&ctrlDataCodecFlags);
	CODEC.Skip(sizeof(DWORD)*5);
	...
	{
		tmpVersion = CODEC.SetVersion( storedVersion );
		CODEC.Read( UD );
		CODEC.Read( &UD->display_alias );
		CODEC.Read( &UD->sound.online );
		CODEC.Read( &UD->sound.message );
		ctrlDataCodecFlags&=0xfff;
		if(ctrlDataCodecFlags>sizeof(UD->ctrlData))
			ctrlDataCodecFlags=sizeof(UD->ctrlData);
		CODEC.ReadDWN((DWORD*)&UD->ctrlData , SIZE_IN_DWORDS(ctrlDataCodecFlags) );
		CODEC.SetVersion( tmpVersion );
	}
}
