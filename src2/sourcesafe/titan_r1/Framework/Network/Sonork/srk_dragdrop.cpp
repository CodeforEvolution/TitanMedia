...
DWORD
 TSonorkClipData::CODEC_DataSize(DWORD codecVersion)	const
{
	DWORD rv;
	rv = sizeof(DWORD);
	switch( D_type )
	{
		...
		case SONORK_CLIP_DATA_USER_DATA:
			rv += D.user_data->CODEC_Size( codecVersion );
			break;
...
{
	if( cf == cfSonorkClipData )
		return CODEC_Size(SONORK_CURRENT_CODEC_VERSION);
	if( cf == CF_TEXT)
 ...
 TSonorkDropSourceData::GetData(CLIPFORMAT cf,void*buffer,DWORD& buffer_size)
{
	...
	if( DD != NULL )
	{
		...
		if( cf == cfSonorkClipData)
		{
			rv = DD->CODEC_WriteMem((BYTE*)buffer,buffer_size,SONORK_CURRENT_CODEC_VERSION) == SONORK_RESULT_OK;
		}
		else
