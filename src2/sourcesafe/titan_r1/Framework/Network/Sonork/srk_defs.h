...
#define SONORK_CLIENT_VERSION_MINOR		6
#define SONORK_CLIENT_VERSION_BUILD		0
#define SONORK_CLIENT_VERSION_PATCH		1

// Server
#define SONORK_SERVER_VERSION_MAJOR		1
#define SONORK_SERVER_VERSION_MINOR		6
#define SONORK_SERVER_VERSION_BUILD		0
#define SONORK_SERVER_VERSION_PATCH		1
...
{
   SONORK_SID_FM1_USER_CONTROLLED	=0x00ffffff
 , SONORK_SID_F1_SONORK_UTS_CLIENT	=0x00001000
 , SONORK_SID_F1_SONORK_UTS_SERVER	=0x00002000
 , SONORK_SID_F1_USOCKS			=0x00004000	// SONORK_SID_F1_USING_SOCKS
 , SONORK_SID_F1_UPORT_MAPPER		=0x00008000	// SONORK_SID_F1_USING_PORT_MAPPER
...
// UTS modes are mapped to SONORK_SID_FLAG2
// and used by
enum SONORK_SID_UTS_MODE
{
   SONORK_SID_UTS_MODE_DIRECT		=0x00000000
,  SONORK_SID_UTS_MODE_CLIENT		=0x00001000
,  SONORK_SID_UTS_MODE_MAPPED		=0x00002000
,  SONORK_SID_UTS_MODE_DISABLED		=0x0000f000
};
enum SONORK_SID_FLAG2
{
   SONORK_SID_FM2_NETWORK_ID		=0x000000ff
,  SONORK_SID_FM2_UTS_MODE		=0x0000f000
};
...
:public SONORK_DWORD4
{
	...
	SONORK_SID_UTS_MODE
		UtsMode() const
		{ return (SONORK_SID_UTS_MODE)(v[1].v[0]&SONORK_SID_FM2_UTS_MODE);}

	void
		SetUtsMode(SONORK_SID_UTS_MODE pV) 
		{ v[1].v[0]=(v[1].v[0]&~SONORK_SID_FM2_UTS_MODE)|(pV&SONORK_SID_FM2_UTS_MODE);}
	...
	void
		SetNetworkId(DWORD netId)
		{ v[1].v[0] = (v[1].v[0]&~SONORK_SID_FM2_NETWORK_ID)|(netId&SONORK_SID_FM2_NETWORK_ID);}

	DWORD
		NetworkId() const
		{ return (v[1].v[0]&SONORK_SID_FM2_NETWORK_ID);}
...
:public SONORK_DWORD2
{
	...
	void
		Set(SONORK_USER_FLAG0 f)
		{ v[0]|=f;}

	void
		Clear(SONORK_USER_FLAG0 f)
		{ v[0]&=~f;}

	BOOL
		Test(SONORK_USER_FLAG0 f)	const
		{ return v[0]&f; }
	...
	void
		Set(SONORK_USER_FLAG1 f)
		{ v[1]|=f;}

	void
		Clear(SONORK_USER_FLAG1 f)
		{ v[1]&=~f;}

	BOOL
		Test(SONORK_USER_FLAG1 f)	const
		{ return v[1]&f; }

	void
		Toggle(SONORK_USER_FLAG1 f)
		{ v[1]^=f; }
...
struct  TSonorkPhysAddr
{
	...
	bool
		IsValid(SONORK_PHYS_ADDR_TYPE t, bool allowZeroPort)	const
		{
			return     (data.inet1.addr.sin_port!=0 || allowZeroPort)
	...
	int
		SetInet1(SONORK_PHYS_ADDR_TYPE addrType,const sockaddr* addr)
		{ return SetInet1( addrType , (const sockaddr_in*)addr); }
...
struct  TSonorkHostInfo
{
	...
	void
		SetV4(const TSonorkPhysAddr*);	// NULL to disable


} __SONORK_PACKED;

struct  TSonorkLocus1
{
	TSonorkId 	userId;
	TSonorkSid	sid;
...
struct  TSonorkUserLocus
:public TSonorkLocus1
{
	...
	void
		SetSidMode(SONORK_SID_MODE mode)
		{  sidFlags.SetSidMode(mode); }

	BOOL
		IsPublic() const
		{ return sidFlags.IsPublic();	}
	...
	SONORK_SID_UTS_MODE
		UtsMode() const
		{ return sidFlags.UtsMode();}

	void
		SetUtsMode(SONORK_SID_UTS_MODE pV)
		{ sidFlags.SetUtsMode(pV);}
	...
	void
		DisableUtsServer()
		{ sidFlags.DisableUtsServer();	}

	void
		DisableUtsClient()
		{ sidFlags.DisableUtsClient();	}
	...
	void
		DisableTracker()
		{ sidFlags.DisableTracker();	}

	BOOL
		IsPrivate() const
		{ return sidFlags.IsPrivate();	}

	BOOL
		UtsServerEnabled() const
		{ return sidFlags.UtsServerEnabled(); }

	BOOL
		UtsClientEnabled() const
		{ return sidFlags.UtsClientEnabled(); }

	BOOL
		UtsEnabled() const
		{ return sidFlags.UtsEnabled(); }

	BOOL
		TrackerEnabled() const
		{ return sidFlags.TrackerEnabled();}

	BOOL
		IsRobot() const
		{ return sidFlags.IsRobot();	}

	BOOL
		IsHuman() const
		{ return sidFlags.IsHuman();}

	BOOL
		IsAdministrator() const
		{ return sidFlags.IsAdministrator();	}

	BOOL
		IsOnline() const
		{ return sidFlags.IsOnline();}

	BOOL
		IsOffline() const
		{ return sidFlags.IsOffline();}

	void Set(const TSonorkUserLocus& O)
		{ memcpy(this,&O,sizeof(*this));}


}__SONORK_PACKED;

struct TSonorkUserLocusEx
:public TSonorkUserLocus
{
	TSonorkPhysAddr	utsAddr;
	TSonorkPhysAddr	hostAddr;

	void
		Set(const TSonorkUserLocusEx& O)
		{ memcpy(this,&O,sizeof(*this));}
...
// ************ OBSOLETE LOCUS/USER ADDRESS STRUCTURES **********************

struct  TSonorkOldLocus2
:public TSonorkLocus1
{
	TSonorkSidFlags		sidFlags;
	TSonorkPhysAddr		physAddr;
...
struct  TSonorkOldLocus3
:public TSonorkOldLocus2
{
	TSonorkRegion	region;
	DWORD		reserved[4];

	void
		SetNew(const TSonorkUserLocusEx& newLocus)
		{
			userId.Set(newLocus.userId);
			sid.Set(newLocus.sid);
			sidFlags.Set(newLocus.sidFlags);
			region.Set(newLocus.region);
			physAddr.Set(newLocus.utsAddr);
		}

}__SONORK_PACKED;

struct  TSonorkUserAddrV15
{
	TSonorkSid	 	sid;
	TSonorkSidFlags		sidFlags;
	TSonorkPhysAddr		physAddr;
...
struct  TSonorkUserInfo1
{
	...
	SONORK_SEX
		GetSex() const
		{ return userFlags.GetSex();}

	BOOL
		IsAdministrator() const
		{ return userFlags.IsAdministrator(); }

	BOOL
		IsRobot() const
		{ return userFlags.IsRobot();	}

	BOOL
		IsBroadcaster() const
		{ return userFlags.IsBroadcaster();	}
...
{
// Flags used by service Servers (TSonorkDynServerHeader)
  SONORK_SERVICE_HF_SYSTEMWIDE		=	0x00100000
, SONORK_SERVICE_HF_LOCATOR		=	0x00200000

// Flags used for sytem messages (TSonorkMsg)
, SONORK_SERVICE_HF_SYS_MSG_VERSION	=	0x07000000
, SONORK_SERVICE_HF_SYS_MSG_V0		=	0x00000000
, SONORK_SERVICE_HF_SYS_MSG_V1		=	0x01000000

// Flags used for UTS descriptors (TSonorkP2Descritor)
// UTS Server will only accept one instance of a user
, SONORK_SERVICE_HF_UTS_SINGLE_USER	=	0x01000000
...
:public TSonorkLocus1
{
	TSonorkServiceInstanceInfo	serviceInfo;
	...
	DWORD
		ServiceHFlags()	const
		{ return serviceInfo.ServiceHFlags();	}
	...
	BOOL
		...
		{ return serviceInfo.ServiceHFlags()&SONORK_SERVICE_HF_SYSTEMWIDE; }

	BOOL
		IsLocatorService() const
		{ return serviceInfo.ServiceHFlags()&SONORK_SERVICE_HF_LOCATOR; }
	...
	void
		...
		{
			serviceInfo.SetInstanceInfo(service_id
				, service_type
				, service_hflags
				, server_instance
				, server_version);
	...
	void
		...
		{
			serviceInfo.SetInfo(si,st,sf);
		}

	void	SetInstance(DWORD instance,DWORD version)
		{
			serviceInfo.SetInstance(instance,version);
		}
...
public:
	...
	SONORK_C_CSTR
		SetMax(SONORK_C_CSTR,UINT maxLen);
...
,   SONORK_PCF_FREE_1			=0x00004000	// SONORK_PCF_USING_PROXY
...
struct TSonorkOldUserServerHeader
{
	struct T_USERVER{
		TSonorkRegion   region;
		DWORD		utsFlags;
...
struct TSonorkNewUserServerHeader
:public TSonorkOldUserServerHeader
{
	TSonorkSidFlags		sidFlags;
}__SONORK_PACKED;

struct TSonorkNewUserServerAddr
:public TSonorkNewUserServerHeader
{
	TSonorkPhysAddr			utsAddr;
	TSonorkPhysAddr			hostAddr;
}__SONORK_PACKED;
...
{
  SONORK_CTRLMSG_CF_01			= 0x08000000
, SONORK_CTRLMSG_CF_02			= 0x04000000
, SONORK_CTRLMSG_CF_03			= 0x02000000
, SONORK_CTRLMSG_CF_04			= 0x01000000
, SONORK_CTRLMSG_CF_05			= 0x00800000
, SONORK_CTRLMSG_CF_06			= 0x00400000
, SONORK_CTRLMSG_CF_07			= 0x00200000
, SONORK_CTRLMSG_CF_08			= 0x00100000
// Some commonly used aliases for standard commands
, SONORK_CTRLMSG_CF_ACCEPT		= SONORK_CTRLMSG_CF_08	// 0x00100000
, SONORK_CTRLMSG_CF_DATA		= SONORK_CTRLMSG_CF_07	// 0x00200000
, SONORK_CTRLMSG_CF_RESULT		= SONORK_CTRLMSG_CF_06	// 0x00400000
};
...
// Note on TSonorkCtrlMsgs:
//  Sonork applications use control messages to quickly exchange
//  information. Control messages are never stored by the server and
//  have lower processing overhead. Other than the header, a control msg
//  does not have a defined structure/format, so the following notes
//  suggest a way of using the common header so that messages unknown by
//  other applications, can still be gracefully processed.
// When starting a query, the sonork applications should send their version as
//  the low word of  UserParam1() and the format in which they are requesting
//  the information should be a SONORK_SERVICE_TYPE in the high word of
//  UserParam1().
// If a fixed structure is passed as data, the low word of UserParam2()
//  should specify the size of the structure. There is no need to pass the size of
//  the whole CtrlMsg packet, because the other always knows, so this
//  applies only when passing fixed structures prefixed to dynamic data.
// Additional flags/values, which are dependant on the service being queried,
//  may be added to the command as SONORK_CTRLMSG_CMD_FLAGS values or the
//  the high word of the UserParam2()
// There are some standard flags used by a control message, which are:
// SONORK_CTRLMSG_CF_ACCEPT (SONORK_CTRLMSG_CF_08)
//  The other side has accepted the request, will answer (or is answering)
// SONORK_CTRLMSG_CF_DATA (SONORK_CTRLMSG_CF_07)
//  Used to indicate that the message contains the data requested.
//  If <SONORK_CTRLMSG_CF_DATA> is missing, but <SONORK_CTRLMSG_CF_ACCEPT>
//  is presend, the service is reporting that it has accepted the command
//  and data will follow.
// SONORK_CTRLMSG_CF_RESULT (SONORK_CTRLMSG_CF_06)
//  Indicates that the service has ended processing the request and is
//  sending the result (as a SONORK_RESULT) in the low word of the
//  UserParam1()
//
...
public:
	...
	DWORD
		UserParam1() const
		{ return body.n.usrParam.v[0]; }

	DWORD
		UserParam2() const
		{ return body.n.usrParam.v[1]; }
...
enum SONORK_ATOM_TYPE
{
	...
	,	SONORK_ATOM_LOCUS			// 147
	,	SONORK_ATOM_SERVICE_LOCUS		// 148
	,	SONORK_ATOM_USER_ADDR			// 149
	,	SONORK_ATOM_P2_DESCRIPTOR		// 150
	,	SONORK_ATOM_P2_MAPPING			// 151
...
#if defined(SONORK_WIN32_BUILD)
enum SONORK_OPEN_FILE_FLAGS
{
   SONORK_OPEN_FILE_F_DELETE_ON_CLOSE = FILE_FLAG_DELETE_ON_CLOSE
,  SONORK_OPEN_FILE_F_RANDOM_ACCESS   = FILE_FLAG_RANDOM_ACCESS
,  SONORK_OPEN_FILE_F_SEQUENTIAL_SCAN = FILE_FLAG_SEQUENTIAL_SCAN
,  SONORK_OPEN_FILE_F_WRITE_THROUGH   = FILE_FLAG_WRITE_THROUGH
};
#endif
