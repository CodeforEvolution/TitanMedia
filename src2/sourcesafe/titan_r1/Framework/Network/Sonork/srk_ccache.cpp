...
SONORK_RESULT TSonorkCCache::Open(SONORK_C_CSTR path , bool deleteOnClose)
{
	if( path == NULL )
	...
	{
		CloseCacheFile( true );
		last_result = OpenCacheFile( path , deleteOnClose);
	}
	return last_result;
}
...
SONORK_RESULT
 TSonorkCCache::OpenCacheFile(const char *pPathBase , bool deleteOnClose)
{
	...
	file.handle=SONORK_IO_OpenFile(
				  ...
				  ,&file_result
				  ,deleteOnClose
				    ?SONORK_OPEN_FILE_F_DELETE_ON_CLOSE
				    :0);
