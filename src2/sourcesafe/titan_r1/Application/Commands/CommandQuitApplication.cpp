...
void CommandQuitApplication::ExecuteE()
{
	if(EMSettingsRepository::Instance() -> SettingExists(SETTING_PROJECT_ID)) // If a project is open
	{
		...
		if(vSaveNeeded)
		{
			switch(m_opMsgBox -> DisplayDialog(NULL/*m_opTrackWindow -> GetWindow() -> GetNativeView()*/, "Do you want to save changes?", "Save Changes?", EM_DIALOG_ICON_QUESTION | EM_DIALOG_BUTTON_YES_NO_CANCEL))
			{
